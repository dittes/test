...
function mountTouch(root, cleanups) {
  base(root, "Touchscreen test", "Tap or move on the test surface to see the pointer events this browser receives.", `
    ...
    <div class="measurement-grid" aria-label="Touchscreen results">
      <div class="measurement"><span class="measurement__label">Pointer type</span><strong data-touch-type>—</strong></div>
  ...
  const position = root.querySelector('[data-touch-position]');
  const result = root.querySelector('[data-touch-result]');
  const type = root.querySelector('[data-touch-type]');
  const state = root.querySelector('[data-touch-state]');
  const status = root.querySelector('[data-status]');
  const update = (event, nextState) => {
...
function mountGamepad(root, cleanups) {
  base(root, 'Gamepad test', 'Ask the browser to check for a connected controller, then press a button or move a control.', `
    <div class="tool-stage gamepad-stage">
      <div class="tool-controls"><button type="button" class="button" data-action="scan">Check for controller</button><button type="button" class="button-secondary button-small" data-action="reset">Reset view</button></div>
      <div class="measurement-grid" aria-label="Controller results">
        <div class="measurement"><span class="measurement__label">Controller</span><strong data-gamepad-name>Not detected</strong></div>
        <div class="measurement"><span class="measurement__label">Last button</span><strong data-gamepad-button>—</strong></div>
        <div class="measurement"><span class="measurement__label">Last axis</span><strong data-gamepad-axis>—</strong></div>
      </div>
    </div>
    <p class="help-note">Only controller input that the browser exposes through the Gamepad API can appear here. Pressing a controller button may be needed before the browser lists it.</p>
    <p class="status" data-status role="status" aria-live="polite">Choose Check for controller, then press a controller button.</p>`);
  const name = root.querySelector('[data-gamepad-name]');
  const button = root.querySelector('[data-gamepad-button]');
  const axis = root.querySelector('[data-gamepad-axis]');
  const status = root.querySelector('[data-status]');
  let frame = 0;
  const read = () => {
    const gamepad = [...(navigator.getGamepads?.() || [])].find(Boolean);
    if (gamepad) {
      name.textContent = gamepad.id || 'Connected controller';
      const pressed = gamepad.buttons.findIndex(item => item.pressed || item.value > 0.1);
      const moved = gamepad.axes.findIndex(value => Math.abs(value) > 0.1);
      if (pressed >= 0) button.textContent = `Button ${pressed}`;
      if (moved >= 0) axis.textContent = `Axis ${moved}: ${gamepad.axes[moved].toFixed(2)}`;
      status.textContent = 'Controller detected. Press a button or move a stick to update the values.';
    } else status.textContent = 'No controller detected yet. Connect it, then press a button and try again.';
    frame = requestAnimationFrame(read);
  };
  const start = () => { cancelAnimationFrame(frame); read(); };
  cleanups.push(attach(root, root.querySelector('[data-action=scan]'), 'click', start));
  cleanups.push(attach(root, window, 'gamepadconnected', start));
  cleanups.push(() => cancelAnimationFrame(frame));
  cleanups.push(attach(root, root.querySelector('[data-action=reset]'), 'click', () => { name.textContent = 'Not detected'; button.textContent = axis.textContent = '—'; status.textContent = 'View reset. Check for controller to start again.'; }));
}
...
export function mountInputTool(root, kind) {
  ...
  if (!["keyboard", "mouse", "screen", "touch", "gamepad"].includes(kind)) throw new RangeError(`Unknown input tool: ${kind}`);
  ...
  else if (kind === "touch") mountTouch(root, cleanups);
  else if (kind === "gamepad") mountGamepad(root, cleanups);
  return dispose;
}
